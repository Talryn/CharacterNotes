...
local CharacterNotes = LibStub("AceAddon-3.0"):NewAddon(addon.addonName, "AceConsole-3.0", "AceHook-3.0", "AceEvent-3.0",
    "AceTimer-3.0")
...
function CharacterNotes:CreateCharNoteTooltip()
    CharNoteTooltip = _G.CreateFrame("GameTooltip", "CharNoteTooltip", _G.UIParent,
        BackdropTemplateMixin and "GameTooltipTemplate,BackdropTemplate" or "GameTooltipTemplate")
...
function CharacterNotes:CreateEditNoteFrame()
    ...
    savebutton:SetScript("OnClick",
        function(this)
            ...
            local note = frame.editbox:GetText()
            local result = self:SaveEditNote(charname, note, rating, editwindow._charname)
    ...
    local charnameText = editwindow:CreateFontString("CN_CharName", "OVERLAY")
    charnameText:SetFont(font, fh + 2, fflags)
    charnameText:SetPoint("BOTTOM", headertext, "BOTTOM", 0, -40)

    local charname = _G.CreateFrame("EditBox", nil, editwindow, "InputBoxTemplate")
    charname:SetPoint("TOP", headertext, "BOTTOM", 0, -15)
    charname:SetPoint("LEFT", editwindow, "LEFT", 30, 0)
    charname:SetScript("OnTextChanged", function(this, userUpdated)
        local frame = this:GetParent()
        frame:UpdateNameStatus()
    end)
    ...
    local statusicon = editwindow:CreateTexture()
    statusicon:SetPoint("LEFT", charname, "RIGHT", 10, 0)
    statusicon:SetWidth(15)
    statusicon:SetHeight(15)

    local statusText = editwindow:CreateFontString("CN_StatusText", "OVERLAY")
    statusText:SetFont(font, fh - 1, fflags)
    statusText:SetPoint("LEFT", statusicon, "RIGHT", 8, 0)
    statusText:SetTextColor(1.0, 1.0, 1.0, 1)
    statusText:SetJustifyH("LEFT")
    statusText:SetText("")

    local editToggleButton = _G.CreateFrame("Button", nil, editwindow, "UIPanelButtonTemplate")
    editToggleButton:SetText(L["Edit"])
    editToggleButton:SetWidth(40)
    editToggleButton:SetHeight(25)
    editToggleButton:SetPoint("LEFT", charnameText, "RIGHT", 25, 0)

    editToggleButton:SetScript("OnClick",
        function(this)
            local frame = this:GetParent()
            frame.editName = not self.editName
            frame:Configure()
        end)
    ...
    if not self.db.profile.multilineNotes then
        editbox:SetScript("OnEnterPressed",
            function(this)
                ...
                local note = frame.editbox:GetText()
                local rating = _G.UIDropDownMenu_GetSelectedValue(editwindow.ratingDropdown)
                self:SaveEditNote(charname, note, rating, editwindow._charname)
                frame:Hide()
            end)
    end
    ...
    editwindow.EditMode = EnumUtil.MakeEnum("Add", "Edit")
    editwindow.NameStatus = EnumUtil.MakeEnum(
        "Valid",
        "InvalidFormat",
        "InvalidExists"
    )
    ...
    editwindow.charnameText = charnameText
    ...
    editwindow.statusicon = statusicon
    editwindow.statusText = statusText
    editwindow.editToggleButton = editToggleButton
    editwindow.editMode = editwindow.EditMode.Edit
    editwindow.editName = true

    editwindow.GetNameUpdateStatus = function(self)
        local name = self.charname:GetText()
        local original = self._charname
        if not name or name == "" then return self.NameStatus.InvalidFormat end
        if (original and name ~= original) or (self.editMode == self.EditMode.Add) then
            local note = NotesDB:GetNote(name)
            if note then
                return self.NameStatus.InvalidExists
            end
        end
        return self.NameStatus.Valid
    end

    editwindow.ShowFrame = function(self, mode, editName, name, note, rating)
        self.editMode = mode
        self.editName = editName
        self._charname = name
        self.charname:SetText(name or "")
        self.charnameText:SetText(name or "")
        self.editbox:SetText(note or "")
        self.headertext:SetText(mode == self.EditMode.Add and L["Add Note"] or L["Edit Note"])

        _G.UIDropDownMenu_SetSelectedValue(self.ratingDropdown, rating)
        ...
        if ratingInfo and ratingInfo.title and ratingInfo.color then
            _G.UIDropDownMenu_SetText(
                self.ratingDropdown,
                ratingInfo.color .. ratingInfo.title .. "|r"
            )
        end
    ...
    editwindow.Configure = function(self)
        if self.editName then
            self.charname:Show()
            self.charnameText:Hide()
            self.statusicon:Show()
            self.statusText:Show()
            self.editToggleButton:Hide()
        else
            self.charname:Hide()
            self.charnameText:Show()
            self.statusicon:Hide()
            self.statusText:Hide()
            self.editToggleButton:Show()
        end
    end

    editwindow.UpdateNameStatus = function(self)
        local status = self:GetNameUpdateStatus()
        local icon = self.statusicon
        local statusTextMapping = {
            [editwindow.NameStatus.InvalidExists] = L["Already Exists"],
            [editwindow.NameStatus.InvalidFormat] = L["Invalid Name"]
        }

        local texture = ""
        if status == self.NameStatus.Valid then
            texture = "Interface\\RAIDFRAME\\ReadyCheck-Ready.blp"
            self.savebutton:Enable()
            self.statusText:SetTextColor(1.0, 1.0, 1.0, 1)
            self.statusText:SetText("")
        elseif status == self.NameStatus.InvalidExists or self.NameStatus.InvalidFormat then
            self.savebutton:Disable()
            texture = "Interface\\RAIDFRAME\\ReadyCheck-NotReady.blp"
            self.statusText:SetTextColor(1.0, 0.0, 0.0, 1)
            self.statusText:SetText(statusTextMapping[status] or "")
        end
...
function CharacterNotes:EditNoteHandler(input)
    ...
    if name and #name > 0 then
        local editwindow = addon.editNoteFrame
        name = NotesDB:FormatUnitName(name)

        local charNote, nameFound = NotesDB:GetNote(name)
        local rating = NotesDB:GetRating(nameFound)
        local charName = charNote and nameFound or name
        local mode = (charNote or rating) and editwindow.EditMode.Edit or editwindow.EditMode.Add

        editwindow:ShowFrame(mode, false, charName, charNote, rating or 0)
    end
end

function CharacterNotes:SaveEditNote(name, note, rating, originalName)
    local operation = "update"
    -- Check for name change
    if originalName and name ~= originalName then
        local note = NotesDB:GetNote(name, false)
        if note then
            -- Name changed but conflicts with existing record
            return "conflict"
        end
        operation = "replace"
    end
    ...
    if operation == "replace" then
        NotesDB:DeleteNote(originalName)
        NotesDB:DeleteRating(originalName)
    end
...
function CharacterNotes:OnEnable()
    ...
    addon.editNoteFrame = self:CreateEditNoteFrame()
