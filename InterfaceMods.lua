...
do
    ...
    function OnEnter(self)
        ...
        if entry then
            ...
            elseif type(entry.activityIDs) == "table" then
                activeEntry.activityID = entry.activityIDs[1]
            end
        end
    ...
    function module:Setup()
        ...
        ScrollBoxUtil:OnViewFramesChanged(_G.LFGListFrame.SearchPanel.ScrollBox,
            function(frames) HookAllFrames(frames, hooks) end)
        ScrollBoxUtil:OnViewScrollChanged(_G.LFGListFrame.SearchPanel.ScrollBox, OnScroll)
        ScrollBoxUtil:OnViewFramesChanged(_G.LFGListFrame.ApplicationViewer.ScrollBox,
            function(frames) HookAllFrames(frames, hooks) end)
